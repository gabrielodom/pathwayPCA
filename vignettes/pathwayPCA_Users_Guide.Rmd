...
```{r echo=FALSE}
ovarian_aespcOut <- readRDS("aespcaOut.RDS")
```
...
signifPaths005_char <- 
  ...
  filter(FDR_BH < 0.05) %>% 
  # Of the remaining data, select the column of pathway titles,
  select(terms) %>% 
  # turn that column into a vector,
  unlist %>% 
  # and remove the duplicated names.
  unname

# Print the names of the significant pathways
signifPaths005_char
```
...
```{r WP75_data}
trimmed_PC[[signifPaths005_char[1]]]$Description
...
Because Wikipathways pathway collections currently store the full names of their pathways in the `description` field, we will need to match the abbreviation for the pathway with the name of the pathway. If the pathway collection you choose has the proper names of the pathways in the `TERMS` field instead of the `description` field, this step is unnecessary. Also, some of the pathway names are quite long, so we truncate them for better graphical display.
```{r match_abbrev_to_name}
# Pair TERMS to description
pathMatch_df <- tibble(
  terms     = trimmed_PC$TERMS,
  pathNames = trimmed_PC$description,
  intKey    = names(trimmed_PC$pathways)
)
...
pathMatch_df <- 
  ...
  mutate(
    pathNames = ifelse(
      str_length(pathNames) > 30,
      paste0(str_sub(pathNames, 1, 28), "..."),
      pathNames
    )
  )
```
...
Now we match the pathway abbreviations to their full names, subset the top 15 pathways by unadjusted $p$-value, and [gather](https://rpubs.com/mm-c/gather-and-spread) the results data frame from "wide" to "tidy" form.
```{r join_and_gather}
pVals_df <- 
  # Results data frame
  ovarian_aespcOut$pVals_df %>% 
...
pathMatch_df %>% 
  filter(terms %in% c(signifPaths005_char, signifPaths010_char))
```
