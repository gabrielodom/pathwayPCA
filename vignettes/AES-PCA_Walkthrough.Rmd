---
title: "AES-PCA Walkthrough with pathwayPCA"
author: "Gabriel Odom"
date: "`r Sys.Date()`"
output:
...
The information stored in the gene set list is very important to our analysis, and the form of this information must be discussed carefully. This gene set list has class `pathwaySet` and contains the following components
...
### Re-inspect the `pathwaySet` List
As we mentioned above, the gene set list (as a `pathwaySet` object, or a gene set list you've created yourself) will be modified upon `Omics*` object creation. Before, this list only had two elements, `pathways` and `TERMS`. Now, it has a third element: `setsize`---the number of genes contained in each pathway.
...
2. Test their association with the response matrix or vector. This step uses a permutation test to estimate the pathway $p$-values.
...
```{r surv_pvals}
a <- Sys.time()
surv_pVals_df <- AESPCA_pVals(object = colon_OmicsSurv,
                              numReps = 1000,
                              parallel = TRUE,
                              numCores = 2,
                              adjustpValues = TRUE,
...
```{r reg_pvals}
b <- Sys.time()
reg_pVals_df <- AESPCA_pVals(object = colon_OmicsReg,
                             numReps = 1000,
                             parallel = TRUE,
                             numCores = 2,
                             adjustpValues = TRUE,
...
```{r categ_pvals}
d <- Sys.time()
categ_pVals_df <- AESPCA_pVals(object = colon_OmicsCateg,
                               numReps = 1000,
                               parallel = TRUE,
                               numCores = 2,
                               adjustpValues = TRUE,
