...
4. Adjust the pathway $p$-values for False Discovery Rate (FDR) or Family-wise Error Rate (FWER).
5. Return a sorted data frame of the adjusted $p$-values for each pathway.

The major differences between the AES-PCA and Supervised PCA methods involve the execution of (2) and (3), which we will describe in their respective methods sections.
...
```
The AES-PCA and Supervised PCA functions give some messages concerning the setup and progress of the computation. The first three sentences inform you of how strong the overlap is between the genes measured in your data and the genes selected in your pathways list. This messages tells us that 9% of the 676 total genes included in all pathways were not measured in the assay; zero pathways were removed from the pathways list for having too few genes after gene trimming; and the genes in the pathways list call for 93.8% of the 656 genes measured in the assay. The last number is the most important: it measures how well your pathways list matches the genes measured in your assay. This number should be as close to 100% as possible. These diagnostic messages depend on the overlap between the pathways list and the assay, so they will be the same across response types or PCA methods.
...
For the two data frames containing the survival data pathway $p$-values, we will transform the data for better graphics. This code takes in the pathway $p$-values data frames (`surv_aes_pVals_df` and `surv_spr_pVals_df`) from the two PCA methods and modifies ("gathers") them into data frames compatible with `ggplot` (`surv_aes_gather_df` and `surv_spr_gather_df`, respectively). We also add on a column for the negative natural logarithm of the pathway $p$-values (called `score`) and recode the label of the $p$-value adjustment method.
```{r subset_top_genes}
