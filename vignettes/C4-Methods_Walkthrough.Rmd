...
1. "Trim" the pathway gene set list.
2. Extract the first principal components (PCs) from each expressed pathway in the assay design matrix.
3. Test the association between the extracted PCs and the response matrix (survival) or vector (all others).
4. Adjust the pathway $p$-values for False Discovery Rate (FDR).
5. Return a sorted data frame of the adjusted $p$-values for each pathway.
...
The details of this step will depend on the method, but the overall idea remains the same. First, select the columns of the assay data frame that correspond to each of the clean/trimmed pathways. Then, use the chosen PCA method to extract the first PCs from those pathways.
...
- `pathways`: The names of the pathways in the `Omics*` object (the names of the `object@@pathwaySet$pathways` list).
...
Now that we have the pathway-specific $p$-values, we can inspect the top pathways ordered by significance. Also, we will plot the most significant $p$-values using the `ggplot2::` package. In order to use the `ggplot` function, we must first "tidy" the data frame returned by the `superPCA_pVals()` and `AESPCA_pVals()` functions. For graphics and data tidying, we will need the suite of `tidyverse` packages:
```{r graphcs_packages}
library(tidyverse)
```

## 5.1 Analysis Function Output
For a quick and easy view of the pathway significance testing results, we can simply print the output data frame. (Note: if you are not using the `tidyverse` package suite, your results will print differently. Use the `head` function to print the top pathways instead.)
```{r viewPathwayRanks}
surv_aes_pVals_df
```

## 5.2 Tidy the Pathway Results
For the two data frames containing the survival data pathway $p$-values, we will transform the data for better graphics. This code takes in the pathway $p$-values data frames from the two PCA methods and modifies ("gathers") them to be compatible with `ggplot`.
```{r subset_top_genes}
surv_aes_gather_df <- surv_aes_pVals_df %>%
  select(-pathways, - setsize, -trim_size) %>%
  gather(variable, value, -terms) %>%
  mutate(score = -log(value)) %>%
  mutate(variable = factor(variable, labels = c("Hochberg", "None", "Sidak SD")))
...
<!--   mutate(variable = factor(variable, labels = c("None", "Hochberg FDR", "Sidak SD FWER"))) %>%  -->
<!--   as.tibble() -->
...
Now we will plot the pathway $p$-values for the most significant pathways as a horizontal bar chart. The thick vertical black line is at $p = 0.05$. This figure shows that a few of the simulated pathways are significant at the $\alpha = 0.05$ for either the Hochberg or Sidak FWER approaches.

```{r surv_aes_pval_plot, fig.height = 6, fig.width = 10.7, out.width = "100%", out.height = "60%"}
ggplot(surv_aes_gather_df) +
  theme_bw() +
