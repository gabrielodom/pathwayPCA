...
1. Describe the components of the `Omics` object class.
...
3. Your response and assay data have already been ID-matched.
...
## 2.1 Class Overview
In this package, all primary input data will be in an `Omics` data object. There are three classes of `Omics*` objects, but one function (`create_Omics`) creates all of them. Each class contains a tidy assay and `pathwaySet` gene set list. The classes differ in the type of response information they can hold. The classes, and their responses, are
...
The `create_Omics` function puts the response information into specific classes:
...
All new `Omics` objects are created with the `create_Omics` function. You should use this function to create `Omics`-class objects for survival, regression, or categorical responses. This `create_Omics` function *internally* calls on a specific creation function for each response type:
  ...
  - Survival response: the `create_OmicsSurv()` function creates an `Omics` object with class `OmicsSurv`. This object will contain:
    ...
    + `assayData_df`: a tidy `data.frame` or `tibble` of prediction information. Rows are observations or subjects; the columns are -Omics measures (e.g. transcriptome). The column names *must* match a subset of the genes provided in the gene set list (in the `pathwaySet` object).
  ...
  - Regression response: the `create_OmicsReg()` function creates an `Omics` object with class `OmicsReg`. This object will contain:
    ...
    + `assayData_df`: a tidy `data.frame` or `tibble` of prediction information, as described above.
...
| `getPathwaySet(object)`          | All               | Extract the `pathwaySet` list stored in `object`.          |
