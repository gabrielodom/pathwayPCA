...
If there are any pathways that are removed due to this execution, the `pathways` list within the `pathwaySet` object within each `Omics*` object will have a character vector of the pathways removed stored as the `"missingPaths"` attribute. Access this attribute with the `attr()` function. As we can see, none of the pathways we started with have been removed.
```{r missingPaths}
...
```{r surv_pvals, cache=TRUE}
a <- Sys.time()
surv_pVals_df <- superPCA_pVals(object = tumourClean_OmicsSurv,
                                parallel = TRUE,
...
```{r reg_pvals, cache=TRUE}
b <- Sys.time()
reg_pVals_df <- superPCA_pVals(object = tumourClean_OmicsReg,
                               parallel = TRUE,
                               numCores = detectCores() - 2,
                               adjustpValues = TRUE,
...
```{r categ_pvals, cache=TRUE}
d <- Sys.time()
categ_pVals_df <- superPCA_pVals(object = tumourClean_OmicsCateg,
                                 parallel = TRUE,
                                 numCores = detectCores() - 2,
                                 adjustpValues = TRUE,
