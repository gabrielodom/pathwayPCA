...
To see the current work on the project, please visit [our package development site](https://github.com/gabrielodom/pathwayPCA) on GitHub, and [our package website](https://gabrielodom.github.io/pathwayPCA/). We can build the PDF manual via
```
path <- find.package("pathwayPCA")
system2(paste(shQuote(file.path(R.home("bin"), "R")),
...
1. `pathways`: a list of the hundreds or thousands of pathways in the pathways list. Each element of the list will be a character vector containing the names of the genes or proteins. For example, a list entry could be
```
...
While we encourage the end-users to supply a pathway, this argument will default to either the "GO" or "MSigDB" pathways. This default pathways list is a topic of further discussion. In future versions of this package, we will attempt to have these pathways automatically update from a web source as the package is loaded or attached via the `.onLoad()` or `.onAttach()` namespace hooks, respectively. 
...
5. Curate a small, example data set and gene pathways list for our examples and vignettes. DONE
...
~~I still need to add "do not touch" examples to the other exported functions. The end user should know that these functions are being called, and how the functions work in detail (should they desire), but should not be encouraged to call them directly.~~ DONE.
    
3. ~~Replace all `\$ stuff \$` with `\eqn\{ stuff \}` in the R documentation files.~~ DONE.
4. Finish documentation for
    a. ~~`aespca()`~~ DONE.
    b. ~~`normalize()`~~ DONE.
5. Add an option to `extract_aesPCs()` to ask the `aespca()` function to return the `oldscore` matrix instead of the `score` matrix. This is all we need to add vanilla PCA to the package. Of course, we'll also have to document this as well, but w/e.
...
### Pathways List
<span style="color:blue"> Do we need “setsize” at all for input? I think we can calculate this one, and return this plus the ratio of number of available assayed features/genes to input pathways list. </span>
...
# Website
Please see the "Articles" page of https://gabrielodom.github.io/pathwayPCA/ for our hosted vignettes. Update the design of the website by editing the `_pkgdown.yaml` file. Update the content of the website pages by editing the vignettes and / or function documentation. When you update the website, run `pkgdown::clean_site()` then rebuild with `pkgdown::build_site()`. In order to check the individual components (like, References, for instance), use `pkgdown::build_reference()` (or whichever section you need). For the available functions, see http://pkgdown.r-lib.org/reference/index.html.
